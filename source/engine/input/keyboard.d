...
public:
    ...
    /**
        Constructor
    */
    this(ubyte* arr, int length) {
        keys[0..length] = cast(bool[])arr[0..length].dup;
    }
...
*/
KeyboardState* fcKeyboardGetState() {
    int* elements = new int;
    ubyte* arr = SDL_GetKeyboardState(elements);

    // Return backup state
    if (elements is null) return bstate;

    // Get state and send it to the state struct
    bstate = new KeyboardState(arr, *elements);
    return bstate;
}